...
const loadingLoginFirstTime = ref<boolean>(false);
const toast = useToast();

onMounted(async () => {
  loadingLoginFirstTime.value = true;
  await authStore.fetchUser();
  if (authStore.user && authStore.isAuthenticated) {
    router.push("/");
    toast.success("You'are already authenticated");
  }
  loadingLoginFirstTime.value = false;
});
</script>

<template>
  <template v-if="!loadingLoginFirstTime">
    <CardAuthLayout>
      <div class="flex-1 flex flex-col justify-center items-center p-6">
        <form
          @submit.prevent="login(formData)"
          class="w-[90vmin] md:w-[60vmin]"
        >
          <div
